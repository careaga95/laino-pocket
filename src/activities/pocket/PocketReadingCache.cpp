...
constexpr char CACHE_MAGIC[] = "LPREAD1";
...
const char* pathFor(const CacheSlot slot) { return slot == CacheSlot::A ? READING_SLOT_A_PATH : READING_SLOT_B_PATH; }
...
CacheResult decodeSlot(const uint8_t* header, const size_t headerBytes, const char* payload, const size_t payloadBytes,
  ...
  if (candidate == 0 || payloadLength == 0 || payloadLength > MAX_READING_MANIFEST_JSON_BYTES ||
  ...
  if (validateReadingManifest(payload, payloadLength) != ReadingParseResult::Success) {
...
CacheOutcome PocketReadingCache::inspect(const CacheSlot slot, char* jsonBuffer, const size_t jsonCapacity,
                                         size_t* payloadLength) {
  if (payloadLength != nullptr) *payloadLength = 0;
  if (jsonBuffer == nullptr || jsonCapacity != MAX_READING_MANIFEST_JSON_BYTES + 1) {
  ...
  if (payloadRead <= MAX_READING_MANIFEST_JSON_BYTES) jsonBuffer[payloadRead] = '\0';
...
CacheOutcome PocketReadingCache::loadBest(ReadingManifest& destination) {
  if (!storage.available()) return {CacheResult::SdUnavailable, CacheSlot::None, 0};
  std::unique_ptr<char[]> jsonBuffer(new (std::nothrow) char[MAX_READING_MANIFEST_JSON_BYTES + 1]);
  if (!jsonBuffer) return {CacheResult::ReadFailure, CacheSlot::None, 0};
  const CacheOutcome a = inspect(CacheSlot::A, jsonBuffer.get(), MAX_READING_MANIFEST_JSON_BYTES + 1);
  const CacheOutcome b = inspect(CacheSlot::B, jsonBuffer.get(), MAX_READING_MANIFEST_JSON_BYTES + 1);
  ...
  const CacheOutcome verified =
      inspect(best.slot, jsonBuffer.get(), MAX_READING_MANIFEST_JSON_BYTES + 1, &payloadLength);
  if (verified.result != CacheResult::Success || verified.generation != best.generation) {
    return {CacheResult::ReadFailure, best.slot, 0};
  }
  if (parseReadingManifest(jsonBuffer.get(), payloadLength, destination) != ReadingParseResult::Success) {
    return {CacheResult::InvalidJsonBundle, best.slot, 0};
  }
  return best;
}

CacheOutcome PocketReadingCache::store(const char* json, const size_t jsonLength) {
  if (validateReadingManifest(json, jsonLength) != ReadingParseResult::Success) {
  ...
  const CacheOutcome verified = inspect(target, jsonBuffer.get(), MAX_READING_MANIFEST_JSON_BYTES + 1);
